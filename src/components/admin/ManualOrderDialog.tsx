  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    ...
    <DialogContent className="max-h-[90vh] overflow-y-auto border-stone-200 bg-white text-stone-950 sm:max-w-2xl">
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="space-y-3 rounded-lg border border-stone-200 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><Label>Order items *</Label><p className="mt-1 text-xs text-stone-500">The Duo includes one ٣١٣ and one ٤٢٤ at AED 220.</p></div><div className="flex gap-2"><Button type="button" size="sm" variant="outline" className="border-stone-300 bg-white !text-stone-900 hover:bg-stone-100" onClick={() => setLines((current) => [...current, makeLine('signature_duo')])}><Plus className="mr-1 h-3.5 w-3.5" /> Duo</Button><Button type="button" size="sm" variant="outline" className="border-stone-300 bg-white !text-stone-900 hover:bg-stone-100" onClick={() => setLines((current) => [...current, makeLine('perfume')])}><Plus className="mr-1 h-3.5 w-3.5" /> Perfume</Button></div></div>
        ...
        <div className="flex items-center justify-between rounded-md bg-stone-900 px-4 py-3 !text-white"><span className="text-sm !text-stone-300">Order total</span><span className="text-lg font-semibold !text-[#d4b483]">AED {total.toFixed(2)}</span></div>
