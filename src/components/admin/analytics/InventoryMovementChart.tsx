              ...
              <button
                ...
                className={`rounded px-2.5 py-1 text-[11px] font-medium capitalize transition ${
                  (stacked && m === 'stacked') || (!stacked && m === 'grouped')
                    ? 'bg-gold text-dark' : 'text-gray-500 hover:text-gold'
                }`}
              >
                {m}
              </button>
            ...
            <div className="group relative flex gap-1 rounded-md border border-gray-200 p-1">
              {(['linear', 'log'] as const).map((m) => (
                <button
                  ...
                  onClick={() => setScale(m)}
                  className={`rounded px-2.5 py-1 text-[11px] font-medium capitalize transition ${
                    scale === m ? 'bg-gold text-dark' : 'text-gray-500 hover:text-gold'
                  }`}
                >
                  {m}
                </button>
              ))}
              <span className="pointer-events-none absolute right-0 top-full z-10 mt-1 hidden w-56 rounded-md border border-gray-200 bg-dark p-2 text-[10px] text-gold/80 shadow-lg group-hover:block">
