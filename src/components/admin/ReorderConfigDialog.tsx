  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-darker border-gold/20">
        <DialogHeader>
          <DialogTitle className="text-gold">Reorder Settings — {perfumeName}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div>
            <Label className="text-gold/80">Lead Time (days)</Label>
            <Input type="number" min={0} value={lead} onChange={(e) => setLead(Number(e.target.value))} className="bg-dark border-gold/30 mt-1" />
          ...
          <div>
            ...
            <Input type="number" min={0} value={safety} onChange={(e) => setSafety(Number(e.target.value))} className="bg-dark border-gold/30 mt-1" />
        ...
        <DialogFooter>
          ...
          <Button onClick={save} disabled={saving} className="bg-gold text-darker hover:bg-gold/90">{saving ? 'Saving…' : 'Save'}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
