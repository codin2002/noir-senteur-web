  ...
  return <nav className={cn('fixed top-0 left-0 right-0 z-50 transition-all duration-300 px-6 md:px-12', isScrolled ? 'py-2 bg-darker bg-opacity-95 backdrop-blur-sm' : 'py-4 bg-transparent')}>
      <div className="flex items-center justify-between">
        ...
        <div className="hidden md:flex space-x-8 items-center">
          <button onClick={() => scrollToSection('collection')} className="text-muted-foreground hover:text-gold transition-colors text-sm uppercase tracking-wider">
            Collection
          </button>
          <button onClick={() => scrollToSection('about')} className="text-muted-foreground hover:text-gold transition-colors text-sm uppercase tracking-wider">
            About
          </button>
          ...
          {user ? <DropdownMenu>
              ...
              <DropdownMenuContent className="w-56 bg-darker text-white border-gold/20">
                <DropdownMenuGroup>
                  <DropdownMenuItem className="focus:bg-gold/10 focus:text-white cursor-pointer" onClick={() => navigate('/profile')}>
                    <User className="mr-2 h-4 w-4" />
                    <span>Profile</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem className="focus:bg-gold/10 focus:text-white cursor-pointer" onClick={() => navigate('/profile?tab=history')}>
                    <History className="mr-2 h-4 w-4" />
                    <span>Purchase History</span>
                  </DropdownMenuItem>
                </DropdownMenuGroup>
                <DropdownMenuSeparator className="bg-gold/20" />
                <DropdownMenuItem className="focus:bg-gold/10 focus:text-white cursor-pointer" onClick={handleSignOut}>
                  Sign Out
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu> : <Button variant="outline" onClick={handleSignIn} className="border-gold text-gold hover:bg-gold hover:text-darker">
              Sign In
            </Button>}
        </div>
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && <div className="md:hidden absolute top-full left-0 right-0 bg-darker bg-opacity-95 backdrop-blur-sm py-4">
          <div className="flex flex-col space-y-4 px-6">
            ...
            {user ? <>
                ...
                <Button variant="outline" onClick={handleSignOut} className="border-gold text-gold hover:bg-gold hover:text-darker">
                  Sign Out
                </Button>
              </> : <Button variant="outline" onClick={handleSignIn} className="border-gold text-gold hover:bg-gold hover:text-darker">
                Sign In
              </Button>}
          </div>
        </div>}
